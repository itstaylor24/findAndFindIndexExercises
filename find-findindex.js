...
Write a function called `findUserByUsername`
 which accepts an array of objects, each with a key of
...
function removeUser(usersArray, username) {
  let foundIndex = usersArray.findIndex(function (user) {
    return user.username === username;
    // will look for a specific user from a username and the key value pair will be returned
  });
  if (foundIndex === -1) return;

  // if the obhect is not found then undefined will be returned
  return usersArray.splice(foundIndex, 1)[0];
  // otherwise, splice at the index--splice changes the array
}
